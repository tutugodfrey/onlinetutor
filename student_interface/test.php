...
$select_result = select_option($course_details, "course code", "course_id");
$display = <<<block
<p>Please select a course to see which tests are avialable</p>
<form name = "testForm" method = "POST" action = "$_SERVER[PHP_SELF]" >
$select_result
<input type = "submit" class = "inner_btns"  id = "viewTest" name = "view_tests" value = "View Tests/Exams" />
</form>
block;
}
...
run_query($query_string, $lecturer_db);
if($row_num2 == 0){
$display = "<p>The courses for this course could not be fetched</p>";
}	else	{
...
$heading = "<h1>Avialable Tests/Exams for $course_code</h1>";
$display = <<<block
<p>Select a test to view test specification and start test</p>
<form  name = "testForm" method = "POST" action = "$_SERVER[PHP_SELF]" >
<input type = "hidden" name = "course_id" value = "$course_id" />
$table_values
<input type = "submit" class = "inner_btns" id = "startTest" name = "test_spec"  value = "Test Details" />
</form>
block;
}
}
...
$test_ind = $_POST["test_id"][0];	//a string that contain the id and the type test/exam
...
<input type = "submit" class = "inner_btns" id = "startTest" name = "start_test" value = "Start Test" />
</form>
block;
}
}
...
$heading = "<h1>$course_code $test_type $test_id</h1>";
$display = <<<block
<form name = "test_specification" method = "POST" action = "$_SERVER[PHP_SELF]" >
...
<input type = "hidden" name = "test_type" value = "$test_type" />
$question_html
<input type = "submit" class = "inner_btns" id = "submitTest" name = "submit_test" value = "Submit" />
</form>
block;
}
}
...
if($percentage_score > 50 && $percentage_score <= 80){
