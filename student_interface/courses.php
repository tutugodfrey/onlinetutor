...
$query_string = "select course_id, course_code course_title, unit from courses";		//work on how to view courses discription
run_query($query_string, $lecturer_db);
if($row_num2 == 0){
$display = "<p>No Course have been saved by your lecturer </p>";
}	else	{
...
$per_course = [];
$course_code = get_course_code($course_detail[0], 1, $lecturer_db);
$per_course[0] = $course_detail[0];
$per_course[1] = $course_code;
$per_course[2] = $course_detail[1];
$per_course[3] = $course_detail[2];
$courses[] = $per_course;
}	//end foreach
$fields = array ("course_id", "Course code", "Course Title", "unit");
array_unshift($courses, $fields);
$table_values = mytable($courses, "yes", "no");
$display = <<<block
<form name = "courses_to_register" method = "POST" action = "$_SERVER[PHP_SELF]">
$table_values
<label for = "course_status"> Course Status</label>
<select name = "course_status" >
<option value ="course_status "selected" = "selected">Select</option>
<option value = "core">Core</option>
<option value = "elective">Elective</option>
</select>
<br />
<br />
<input type = "submit" class = "inner_btns" id = "registerCourse"  value = "Register" name = "register" />
...
array_unshift($course_detail, $fields);
$table_values = mytable($course_detail, "yes", "no");
$display = <<<block
