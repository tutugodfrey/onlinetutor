...
if(isset($_GET["results"])){
	...
	if($row_num2 == 0){
		...
		$select_result1 <br />
		$select_result2 <br />
		<input type = "submit" class = "btn btn-success" id = "submittedTest" name = "submitted_test" value = "Submitted Tests" />
		<br />
		<input type = "submit" class = "btn btn-success" id = "totalScores" name = "total_score" value = "Accumulated Scores" />
		</form>
block;
	}
}		//end results
...
if(isset($_GET["submitted_test"])){
	...
	if($course_id == "" || $score_type == "" ){
		...
		if($score_type == "test"){
			$query_string = "select distinct test_id from score_board where course_id = \"$course_id\" and score_type = \"test\" and lec_id = \"$owner_id\"";
		}
		if($score_type == "discussion"){
			$query_string = "select distinct discussion_id from score_board where course_id = \"$course_id\" and score_type = \"discussion\" and lec_id = \"$owner_id\"";
		}
		if($score_type == "exam"){
			$query_string = "select distinct test_id from score_board where course_id = \"$course_id\" and score_type = \"exam\" and lec_id = \"$owner_id\"";
		}
		run_query($query_string);
		if($row_num2 ==  0){
			...
			if($score_type == "test"){
				$query_string = "select distinct student_id from score_board where course_id = \"$course_id\" and score_type = \"test\" and lec_id = \"$owner_id\"";
			}
			if($score_type == "discussion"){
				$query_string = "select distinct student_id from score_board where course_id = \"$course_id\" and score_type = \"discussion\" and lec_id = \"$owner_id\"";
			}
			if($score_type == "exam"){
				$query_string = "select distinct student_id from score_board where course_id = \"$course_id\" and score_type = \"exam\" and lec_id = \"$owner_id\"";
			}
			run_query($query_string);
			if($row_num2 == 0 ){
				...
				foreach($student_ids as $student_id){
					...
					$student_row[0] = $names;
					if($score_type == "test"){
					$query_string = "select test_id, score from score_board where course_id = \"$course_id\" and score_type = \"test\" and student_id = \"$student_id\" and lec_id = \"$owner_id\"";
					}
					if($score_type == "discussion"){
					$query_string = "select discussion_id, score from score_board where course_id = \"$course_id\" and score_type = \"discussion\" and student_id = \"$student_id\" and lec_id = \"$owner_id\"";
					}
					if($score_type == "exam"){
					$query_string = "select test_id, score from score_board where course_id = \"$course_id\" and score_type = \"exam\" and student_id = \"$student_id\" and lec_id = \"$owner_id\"";
					}
					run_query($query_string);
					if($row_num2 == 0){
					...
					if($score_type == "test"){
					$query_string = "select sum(score), avg(score) from score_board where course_id = \"$course_id\" and score_type = \"test\" and student_id = \"$student_id\" and lec_id = \"$owner_id\"";
					}
					if($score_type == "discussion"){
					$query_string = "select sum(score), avg(score) from score_board where course_id = \"$course_id\" and score_type = \"discussion\" and student_id = \"$student_id\" and lec_id = \"$owner_id\"";
					}
					if($score_type == "exam"){
					$query_string = "select sum(score), avg(score) from score_board where course_id = \"$course_id\" and score_type = \"exam\" and student_id = \"$student_id\" and lec_id = \"$owner_id\"";
					}
					run_query($query_string);
					if($row_num2 == 0){
						$avg_score = "";
...
if(isset($_GET["total_score"])){
	...
	if($course_id == ""){
		...
		foreach($student_ids as $student_id){
			...
			$names = get_names($student_id);
			$student_row[0] = $names;
			$query_string = "select score_type, test_id, discussion_id, score from score_board where course_id = \"$course_id\" and student_id = \"$student_id\" and lec_id = \"$owner_id\"";
			run_query($query_string);
			if($row_num2 == 0){
				$display = "no scores";
			}	else	{
			...
			$query_string = "select sum(score), avg(score) from score_board where course_id = \"$course_id\" and student_id = \"$student_id\" and lec_id = \"$owner_id\"";
			run_query($query_string);
			if($row_num2 == 0 ){
				$avg_scores = "no found";
			}	else	{
